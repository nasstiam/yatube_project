...
class TestPostView:
    ...
    def test_post_view_get(self, client, post_with_group):
        ...
        assert profile_context is not None, \
            'Проверьте, что передали автора в контекст страницы `/posts/<post_id>/`'
...
class TestPostEditView:
    ...
    def test_post_edit_view_get(self, client, post_with_group):
        ...
        if response.status_code in (301, 302) and not response.url.startswith(f'/posts/{post_with_group.id}'):
            response = client.get(f'/posts/{post_with_group.id}/edit/')
        assert response.status_code != 404, \
            'Страница `/posts/<post_id>/edit/` не найдена, проверьте этот адрес в *urls.py*'

        assert response.status_code in (301, 302), \
            'Проверьте, что вы переадресуете пользователя со страницы `/posts/<post_id>/edit/` на страницу поста, если он не автор'
    ...
    def test_post_edit_view_author_get(self, user_client, post_with_group):
        ...
        post_context = get_field_context(response.context, Post)
        assert post_context is not None, \
            'Проверьте, что передали статью в контекст страницы `/posts/<post_id>/edit/` типа `Post`'
    ...
    def test_post_edit_view_author_post(self, user_client, post_with_group):
        ...
        try:
            ...
            assert False, f'''Страница `/posts/<post_id>/edit/` работает неправильно. Ошибка: `{e}`'''
        url = f'/posts/{post_with_group.id}/edit/' if response.status_code in (301, 302) else f'/posts/{post_with_group.id}/edit'

        response = user_client.post(url, data={'text': text, 'group': post_with_group.group_id})

        assert response.status_code in (301, 302), \
            'Проверьте, что со страницы `/posts/<post_id>/edit/` после создания поста перенаправляете на страницу profile'
        post = Post.objects.filter(author=post_with_group.author, text=text, group=post_with_group.group).first()
        assert post is not None, \
            'Проверьте, что вы измeнили пост при отправки формы на странице `/posts/<post_id>/edit/`'
        assert response.url.startswith(f'/profile/{post_with_group.author.username}/'),\
            'Проверьте, что перенаправляете на страницу profile `/profile/<username>/`'
