...
class TestNewView:
    ...
    def test_new_view_post(self, user_client, user, group):
        ...
        assert response.status_code in (301, 302), \
            'Проверьте, что со страницы `/new/` после создания поста перенаправляете на страницу пользователя'
        ...
        assert response.url == '/profile/TestUser/', 'Проверьте, что перенаправляете на страницу пользователя`/`'

        text = 'Проверка нового поста 2!'
        response = user_client.post(url, data={'text': text})
        assert response.status_code in (301, 302), \
            'Проверьте, что со страницы `/new/` после создания поста перенаправляете на страницу пользователя'
        post = Post.objects.filter(text=text, group__isnull=True).first()
        assert post is not None, 'Проверьте, что вы сохранили новый пост при отправки формы на странице `/new/`'
        assert response.url == f'/profile/{post.author.username}/', 'Проверьте, что перенаправляете на страницу пользователя`/profile/TestUser/`'
